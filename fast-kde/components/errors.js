...
class ErrorVisualizer extends D3Component {
  initialize(node, props) {
      ...
      function prep(data, bins, grid) {
        const points =  data.filter(d => d.bins === bins && d.grid === grid);
        return Array.from(
      ...
      const paths = svg.selectAll('path.series')
        .data(prep(data, bins, grid), d => d.method)
        .join('path')
...
module.exports = ErrorVisualizer;
